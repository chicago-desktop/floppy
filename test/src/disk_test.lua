...
local function define_tests()
    test.describe("WAPP floppy lifecycle", function()
        test.it("browses named disks without registration and loads user disks after refresh",function()
            local state=window.definition.init()
            test.eq(state.disk_source,"bundled")
            test.is_true(#state.disks>=2)
            test.eq(state.path,"ski.wapp")
            ...
            local picture=assert(render.placement({id="catalog",state_revision=1,content_state={sdk=1,revision=1,ui=tree}},
                {x=1,y=1,cols=52,rows=20},{w=10,h=20},fonts,store))
            assert(assert(fs.get("app:shots")):writefile("disk-catalog.png",assert(picture.raster:encode("png"))))
            window.definition.update(state,{type="activate",id="add_disks"},{})
            test.is_true(state.add_help)
            test.is_nil(ui.problem(window.definition.view(state)))
            window.definition.update(state,{type="activate",id="help_done"},{})
            test.is_false(state.add_help)
            local personal=assert(fs.get("chicago.floppy:library"))
            local bundled=assert(fs.get("chicago.floppy:disks"))
            local name="test-"..state.namespace:match("disk_(.+)$")..".wapp"
            local broken="broken-"..name
            window.definition.update(state,{type="change",id="disk_source",value="personal"},{})
            local count=#state.disks
            assert(personal:writefile(name,assert(bundled:readfile("hello.wapp"))))
            assert(personal:writefile(broken,"not a wapp"))
            test.eq(#state.disks,count,"the catalog refresh is explicit")
            window.definition.update(state,{type="activate",id="refresh_disks"},{})
            local valid_index,bad_index=nil,nil
            for i,disk in ipairs(state.disks) do
                if disk.path==name then valid_index=i;test.eq(disk.title,"Chicago Demo Disk") end
                if disk.path==broken then bad_index=i;test.not_nil(disk.error) end
            end
            test.not_nil(valid_index);test.not_nil(bad_index)
            window.definition.update(state,{type="select",id="disks",index=bad_index},{})
            window.definition.update(state,{type="activate",id="insert"},{})
            test.is_nil(state.package)
            window.definition.update(state,{type="select",id="disks",index=valid_index},{})
            window.definition.update(state,{type="activate",id="insert"},{})
            test.not_nil(state.package,tostring(state.status));test.is_false(state.registered)
            window.definition.dispose(state)
            assert(personal:remove(name));assert(personal:remove(broken))
        end)
        ...
        test.it("discovers A: and runs the lifecycle in a real window process", function()
            ...
            click("insert")
            window.definition.update(layout_state,{type="activate",id="insert"},{})
            ...
            click("eject")
            window.definition.dispose(layout_state)
